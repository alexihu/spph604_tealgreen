...
rm(rx_info, rx_class_joined, rx_classes_wide,rx_classes_wide_name)
...
comorb_vars <- c(
  ...
  # Some additions
  "diq080", # diabetes with retinopathy
  "mcq190", # rheumatoid arthritis 2003/4 to 2007/8 to replace arthritis of any kind ("mcq160a")
  "mcq191", # rheumatoid arthritis 2009/10 to replace arthritis of any kind ("mcq160a")
  "mcq195" # rheumatoid arthritis 2011/12 to 2017/18 to replace arthritis of any kind ("mcq160a")
)
...
comorb_weights <- c(
  ...
  mcq160l = 2,
  mcq220 = 2,
  diq_noret = 1,  # diabetes needs a recode for diq080 and diq010 first
  diq_ret = 2,  # diabetes needs a recode for diq080 and diq010 first
  bpq020 = 0 ,  
  kiq022 = 2,   
  mcq_ra = 1
)
...
# Create diabetes column and a rheumatoid arthritis column
health_wide <- health_wide %>%
 mutate(
    diq_noret = case_when(
      diq010 == "Yes" & diq080 == "No" ~ "Yes",          # diabetes, no retinopathy
      diq010 == "Yes" & diq080 == "Yes" ~ "No",          # diabetes, with retinopathy
      diq010 == "No" ~ "No",                        # no diabetes
      TRUE ~ NA_character_
    ),
    diq_ret = case_when(
      diq010 == "Yes" & diq080 == "Yes" ~ "Yes",          # diabetes, retinopathy
      diq010 == "Yes" & diq080 == "No" ~ "No",          # diabetes, no retinopathy
      diq010 == "No" ~ "No",                        # no diabetes
      TRUE ~ NA_character_
    ),
    mcq_ra = case_when(
        mcq190 == "Rheumatoid arthritis"  ~ "Yes",
        mcq191 == "Rheumatoid arthritis"  ~ "Yes", 
...
# Compute count by sum, calculate weighted sum 
...
mcq_clean <- health_num %>%
  dplyr::mutate(
    ...
    comorbidity_cci = as.numeric(comorb_matrix %*% weights_vec)
  ) %>%
  dplyr::select(seqn, comorbidity_n, comorbidity_cci)
