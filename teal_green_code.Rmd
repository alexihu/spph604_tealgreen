---
title: "SPPH604: Polypharmacy & Mortality – Reproducible Analyses"
...
This document analyzes the association between levels of prescription medication use and all-cause mortality among U.S. adults aged 45 years and older in NHANES 2003–2018. We follow NHANES analytic guidance (design first → subset → analyze), handle missingness in key covariates with multiple imputation, and fit/pool survey-weighted Cox models. We also include spline sensitivity analyses, interaction models with therapeutic classes, and several robustness checks (censoring, multimorbidity restrictions).

**Research question.** Among U.S. adults aged 45+ in NHANES 2003–2018, are different levels of medication use (0, 1–4, 5–9, ≥10) associated with all-cause mortality?

**PICOT**

**Population:** U.S. adults aged ≥45 in NHANES 2003–2018

**Intervention/exposure:** polypharmacy level (by interview-reported medication count)

**Comparator:** 1–4 medications
...
3.  Then **impute both income-to-poverty ratio and insurance** using all analysis variables + survey design vars + a Nelson–Aalen cumulative hazard (so the outcome informs the imputation), following White & Royston (2009).
...
# 4) Eligibility: age ≥ 45, non-missing survival time/event
dsgn_elig <- subset(
  dsgn_all,
...
dsgn_elig <- update(
  ...
  poly_cat_no_abx = factor(poly_cat_no_abx, levels = c("1–4","0","5–9","≥10"))
)
...
desc_df <- elig_df %>%
  mutate(
    ...
    indfmpir_cat_obs = factor(
      indfmpir_cat_obs,
      levels = c("<1.00 (below poverty)", "1.00–1.99", "2.00–3.99", "≥4.00", "Missing")
    ),
...
```{r km-curves, fig.width=9, fig.height=8, fig.cap="Kaplan-Meier survival curves by polypharmacy category with risk table"}
# ---- Survey-weighted Kaplan–Meier by polypharmacy with Aligned Risk Table -----
...
# 2. Extract Data for Plotting (Curve)
km_df <- purrr::imap_dfr(km_fit, function(fit, grp) {
  ...
  mutate(
    # Map raw codes to descriptive labels
    poly_cat = case_when(
      poly_cat_raw == "0"   ~ "0 medications",
      poly_cat_raw == "1–4" ~ "1–4 medications",
      poly_cat_raw == "5–9" ~ "5–9 medications",
      poly_cat_raw == "≥10" ~ "≥10 medications",
      TRUE ~ poly_cat_raw
    ),
    # Ensure correct factor ordering
    poly_cat = factor(poly_cat, levels = c("1–4 medications", "0 medications", "5–9 medications", "≥10 medications"))
  )

# 3. Create Data for Risk Table (Standard Unweighted Counts)
#    We define specific time breaks for the table
table_times <- seq(0, 15, by = 3) # 0, 3, 6, 9, 12, 15

fit_unweighted <- survfit(Surv(time_y, event) ~ poly_cat, data = desc_df)
risk_data <- summary(fit_unweighted, times = table_times, extend = TRUE)
...
risk_tbl_df <- tibble(
  ...
  mutate(
    # Clean up strata names from "poly_cat=1-4" to just the code
    poly_cat_raw = str_remove(strata, "poly_cat="),
    ...
    # Ensure factor order matches the main plot (usually reference 1-4 first or ordered by risk)
    # We maintain the same order as in the KM plot for consistency
    poly_cat = factor(poly_cat, levels = c("1–4 medications", "0 medications", "5–9 medications", "≥10 medications"))
  )

# 4. Set Plot Parameters for Perfect Alignment
#    We use the same X-axis limits and breaks for both
x_limits <- c(0, 17) # Extend slightly past 16
x_breaks <- table_times

# 5. Build the Main KM Plot
p_km <- ggplot(km_df, aes(x = time, y = surv, color = poly_cat)) +
  geom_step(linewidth = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,1)) +
  scale_x_continuous(breaks = x_breaks, limits = x_limits) +
  scale_color_brewer(palette = "Dark2") + # distinct colors
  labs(
    title = "Survey-weighted Kaplan–Meier survival",
    x     = NULL, # Hide X label here, put it on table
    y     = "Survival Probability",
    color = "Medication Use"
  ) +
  theme_classic(base_size = 14) +
  theme(
    legend.position = c(0.25, 0.35), # Place legend inside plot
    legend.background = element_rect(fill = "white", color = "black"),
    axis.text.x = element_blank(), # Hide x text (will be in table)
    axis.ticks.x = element_blank(),
    plot.margin = margin(t=10, r=20, b=5, l=10)
  )

# 6. Build the Risk Table Plot
p_table <- ggplot(risk_tbl_df, aes(x = time, y = poly_cat, label = scales::comma(n.risk))) +
  geom_text(size = 4.5) +
  scale_x_continuous(breaks = x_breaks, limits = x_limits) +
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", hjust = 1, color = "black"),
    axis.text.x = element_text(color = "black"),
    panel.grid = element_blank(),
    plot.margin = margin(t=0, r=20, b=10, l=10) # Align margins with top plot
  )

# 7. Combine with Patchwork
#    heights = c(3, 1) gives 75% height to plot, 25% to table
combined_plot <- p_km / p_table + plot_layout(heights = c(3, 1))

combined_plot

# 8. Design-based omnibus test
...
# **Multiple imputation (indfmpir, insured) with Nelson–Aalen hazard**
...
We now build an imputation model that:
  ...
  - includes a **Nelson–Aalen cumulative hazard** built from the observed survival, as recommended by White & Royston (2009) to ensure the outcome informs the imputation process.
...
```{r mi-nelson-aalen}
# Overall (unweighted) Nelson–Aalen from the eligible sample
...
mk_indfmpir_cat <- function(x) {
  factor(case_when(
    ...
    x >= 2 & x < 4        ~ "2.00–3.99",
    x >= 4                ~ "≥4.00"
  ), levels = c("<1.00 (below poverty)","1.00–1.99","2.00–3.99","≥4.00"))
}
```
...
tbl_combined <- gtsummary::tbl_merge(
  ...
  gtsummary::modify_footnote(
    update = gtsummary::all_stat_cols() ~
      "HR = hazard ratio; CI = confidence interval. Reference category for polypharmacy is 1–4 medications."
  )
...
# ------------------------------------------------------------
# helper: tidy MIcombine output → tibble with term column
# ------------------------------------------------------------
...
fit_interaction_for_flag <- function(flag, imp_obj) {
  ...
  for (i in seq_len(m)) {
    ...
    di <- di %>%
      ...
      dplyr::mutate(
        poly_cat     = factor(poly_cat, levels = c("1–4","5–9","≥10")),
        age_cat      = mk_age_cat(ridageyr),
...
interaction_measures_from_pooled <- function(pooled_obj) {
  ...
  if ("poly_cat5–9" %in% names(beta)) {
    rows[["poly5_9_class0"]] <- get_lhr(c(`poly_cat5–9` = 1))
  }
  if (all(c("poly_cat5–9","class_flag1","poly_cat5–9:class_flag1") %in% names(beta))) {
    rows[["poly5_9_class1"]] <- get_lhr(c(
      `poly_cat5–9` = 1,
      class_flag1 = 1,
      `poly_cat5–9:class_flag1` = 1
    ))
  }
  if ("poly_cat≥10" %in% names(beta)) {
    rows[["poly10p_class0"]] <- get_lhr(c(`poly_cat≥10` = 1))
  }
  if (all(c("poly_cat≥10","class_flag1","poly_cat≥10:class_flag1") %in% names(beta))) {
    rows[["poly10p_class1"]] <- get_lhr(c(
      `poly_cat≥10` = 1,
      class_flag1   = 1,
      `poly_cat≥10:class_flag1` = 1
    ))
  }
...
int_tbls <- purrr::imap(int_results, function(x, flag) {
  ...
  gtsummary::tbl_regression(
    ...
    gtsummary::modify_caption(
      paste0("**Survey-weighted Cox model with polypharmacy × ", flag, " (0-meds excluded)**")
    ) %>%
    gtsummary::bold_labels()
})
sink()
...
# -------------------------------------------------------------------
# Revised plotting logic: Y-axis = User/Non-User; Color = Polypharmacy Level
plot_dat_revised <- plot_dat %>%
  dplyr::mutate(
    # Extract User Status from the combo name (class1 = User, class0 = Non-user)
    user_status = dplyr::case_when(
      stringr::str_detect(combo, "class1") ~ "User",
      stringr::str_detect(combo, "class0") ~ "Non-user",
      TRUE ~ NA_character_
    ),
    # Extract Polypharmacy Level
    poly_group = dplyr::case_when(
      stringr::str_detect(combo, "poly5_9") ~ "5–9 medications",
      stringr::str_detect(combo, "poly10p") ~ "≥10 medications",
      stringr::str_detect(combo, "poly1_4") ~ "1–4 medications",
      TRUE ~ NA_character_
    ),
    # Create Label string: HR [LCL; UCL]
    text_label = sprintf("%.2f [%.2f; %.2f]", HR, LCL, UCL),
    # Clean Flag Names for Facet Titles
  ...
  # Filter: Keep only the high polypharmacy groups for the plot
  dplyr::filter(poly_group %in% c("5–9 medications", "≥10 medications")) %>%
  # Set factor levels for consistent plotting order
  dplyr::mutate(
    user_status = factor(user_status, levels = c("Non-user", "User")),
    poly_group = factor(poly_group, levels = c("5–9 medications", "≥10 medications"))
  )

# Faceted Plot by Therapeutic Class
ggplot(plot_dat_revised,
       aes(x = HR, y = user_status, color = poly_group, group = poly_group)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "grey50") +
  
  # Error bars and points with dodging
  geom_errorbar(aes(xmin = LCL, xmax = UCL),
                width = 0.2, position = position_dodge(width = 0.6), linewidth = 0.8) +
  geom_point(position = position_dodge(width = 0.6), size = 2.5) +
  
  # Add text labels to the right of the error bar
  ...
  scale_x_log10(breaks = c(0.5, 1, 2, 4, 8),
                limits = c(0.2, 16)) + # Adjust limits to fit text labels
...
To assess whether the categorical groupings (0, 1-4, 5-9, ≥10) adequately capture the dose-response relationship, we fit a model using a restricted cubic spline for continuous medication count. This flexible approach allows us to visualize potential non-linear associations without imposing arbitrary cut-points. We use a natural spline with 2 degrees of freedom (equivalent to 3 knots), which provides flexibility while avoiding overfitting.
...
mm_pooled <- purrr::map2_dfr(fit_mm, cutpoints, function(mira_obj, k) {
  tidy_poly(mira_obj, paste0("Comorbidity ≥ ", k))
})
...
mm_plot <- mm_pooled %>%
    mutate(
        exposure_lbl = case_when(
            term == "poly_cat5–9"  ~ "5–9 medications",
            term == "poly_cat≥10"  ~ "≥10 medications",
            term == "poly_cat1–4"  ~ "1–4 medications (ref)",
            term == "poly_cat0"    ~ "0 medications",
            TRUE                   ~ term
        ),
        .keep = "all"
    ) %>%
    filter(exposure_lbl != "1–4 medications (ref)") %>%
    mutate(
        model = fct_rev(fct_inorder(model)),
        exposure_lbl = factor(exposure_lbl,
                              levels = c("0 medications", "5–9 medications", "≥10 medications"))
    )
...
df_plot <- all_res %>%
  ...
  mutate(
    exp.label = case_when(
        ...
        term %in% c("poly_cat5–9", "poly_cat_no_abx5–9") ~ "5–9 medications",
        term %in% c("poly_cat≥10", "poly_cat_no_abx≥10") ~ "≥10 medications",
        TRUE ~ NA_character_
    )
  ) %>%
...
df_viz <- df_viz %>%
  ...
  mutate(exposure_group = factor(exposure_group, 
                                 levels = c("0 medications", "5–9 medications", "≥10 medications", "Header")))
...
plot.forest <- df_viz %>%
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "5–9 medications" = "#0072B2",  # Blue
      "≥10 medications" = "#CC79A7",  # Reddish Purple
      "Header"          = "transparent" 
    ),
    breaks = c("0 medications", "5–9 medications", "≥10 medications")
  ) +
  scale_shape_manual(
    name = "Exposure Group",
    values = c(
      "0 medications"   = 16, # Circle
      "5–9 medications" = 15, # Square
      "≥10 medications" = 17, # Triangle
      "Header"          = NA
    ),
    breaks = c("0 medications", "5–9 medications", "≥10 medications")
  ) +
