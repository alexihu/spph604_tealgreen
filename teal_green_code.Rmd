...
# Quick visual check of convergence (suppressing plot messages)
invisible(plot(imp, c("indfmpir","insured")))
```
...
# **Main Cox models (crude → basic → full)**
...
plot_dat_clean <- plot_dat %>%
  dplyr::mutate(
    combo_lbl = dplyr::case_when(
      combo == "poly1_4_class0"  ~ "1–4 meds, non-user",
      combo == "poly1_4_class1"  ~ "1–4 meds, user",
      combo == "poly5_9_class0"  ~ "5–9 meds, non-user",
      combo == "poly5_9_class1"  ~ "5–9 meds, user",
      combo == "poly10p_class0"  ~ "≥10 meds, non-user",
      combo == "poly10p_class1"  ~ "≥10 meds, user",
      TRUE ~ combo
    ),
...
# **Spline sensitivity: continuous medication count**
...
```{r spline-analysis}
# Fit spline model (relative to 0 medications)
fit_spline <- with(imp, {
  di <- tibble(
    sdmvpsu, sdmvstra, wtint_pool, time_y, event,
    rxdcount_cont = as.numeric(rxdcount),
    age_cat       = mk_age_cat(ridageyr),
    sex, race_eth, ed_cat,
    insured       = factor(insured, levels = c("No","Yes")),
    indfmpir, indfmpir_cat = mk_indfmpir_cat(indfmpir),
  ...
  svycoxph(
    Surv(time_y, event) ~ ns(rxdcount_cont, df = 2) +
      age_cat + sex + race_eth + ed_cat + indfmpir_cat + insured +
      comorbidity_n + cci_score + visits_cat + hosp_stays_cat,
    design = dd
  )
...
sink(tempfile())
# Pool results
po <- mice::pool(fit_spline)

sink()

pooled_summary <- summary(po, conf.int = TRUE)

# Extract coefficients for the spline terms
spline_coefs <- pooled_summary %>%
  filter(grepl("^ns\\(rxdcount_cont", term))

# Check if we have spline terms
if (nrow(spline_coefs) > 0) {
  
  # Generate prediction grid
  rx_seq <- seq(0, 20, by = 0.5)
  
  # Get data from first imputation to establish knot structure
  d1_spline <- mice::complete(imp, 1) %>%
    mutate(rxdcount_cont = as.numeric(rxdcount))
  
  # Fit a reference spline to get knot attributes
  ref_spline <- ns(d1_spline$rxdcount_cont, df = 2)
  knots_attr <- attr(ref_spline, "knots")
  boundary_attr <- attr(ref_spline, "Boundary.knots")
  
  # Generate spline basis for prediction grid using same knots
  basis_pred <- ns(rx_seq, knots = knots_attr, Boundary.knots = boundary_attr)
  
  # Extract spline coefficient vector
  spline_beta <- spline_coefs$estimate
  
  # Get the variance-covariance matrix from the pooled object
  # The pooled vcov matrix is stored in po$pooled$t
  # We need to extract just the spline term variances
  spline_term_names <- spline_coefs$term
  all_term_names <- pooled_summary$term
  spline_indices <- which(all_term_names %in% spline_term_names)
  
  # Extract standard errors for spline terms
  spline_se <- spline_coefs$std.error
  
  # Create a diagonal variance matrix (simplified approach)
  # This is conservative as it ignores covariances between spline terms
  vcov_spline_diag <- diag(spline_se^2)
  
  # Compute linear predictor for spline terms (relative to rxdcount = 0)
  lp <- as.vector(basis_pred %*% spline_beta)
  
  # Compute standard errors (using diagonal variance matrix)
  # This is a conservative estimate
  se_lp <- sqrt(diag(basis_pred %*% vcov_spline_diag %*% t(basis_pred)))
  
  # Create HR curve data
  hr_curve <- tibble(
    rxdcount = rx_seq,
    HR       = exp(lp),
  ...
  # Plot
  ggplot(hr_curve, aes(x = rxdcount, y = HR)) +
    ...
    labs(
      title = "Dose-response relationship: medication count and mortality",
      subtitle = "Natural spline model (df=2, relative to 0 medications)",
  ...
  cat("Note: Spline terms not found in model output. Skipping spline plot.\n")
}
```

**Interpretation:** The spline model reveals the shape of the dose-response relationship between medication count and mortality risk. We observe whether the association is approximately linear or exhibits threshold effects or plateaus. This visualization helps validate our categorical groupings and provides insight into whether particular medication counts correspond to inflection points in mortality risk. The shaded region represents the 95% confidence interval, with wider intervals at the extremes reflecting fewer observations. Note that confidence intervals use a conservative approach that ignores covariances between spline terms, so actual intervals may be slightly narrower.
