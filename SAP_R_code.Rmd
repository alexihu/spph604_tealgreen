...
nhanes_get <- function(table_base, suffix, quiet = FALSE) {
  ...
  out <- tryCatch(nhanesA::nhanes(nm), error = function(e) NULL)
...
demo_roster <- demo_all %>% transmute(seqn = as.integer(seqn)) %>% distinct()

# Tables that contain RXDCOUNT directly (by cycle)
rx_tables <- tryCatch(
...
# table(rx_all$rxdcount == 0)
  
# MCQ: comorbidity items (Yes=1, else 0; sum across items) 
# Requested variables (case-insensitive)
mcq_keep <- tolower(c(
  "SEQN",
  "MCQ010",
  "MCQ160A","MCQ160N","MCQ160B","MCQ160C","MCQ160D","MCQ160E","MCQ160F",
  "MCQ160M","MCQ160G","MCQ160K","MCQ160O",
  "MCA160L",   # included as given
  "MCQ550","MCQ203","MCQ220"
))
# angina pectoris, arthritis, asthma, chronic bronchitis, congestive heart failure, coronary heart disease, diabetes, emphysema, gout, hypertension, liver condition, myocardial infarction, obesity, stroke, thyroid problems, and kidney disease

mcq_all <- purrr::map_dfr(cycle_meta$suffix, ~{
  df <- nhanes_get("MCQ", .x, quiet = TRUE); if (is.null(df)) return(tibble())
  df <- janitor::clean_names(df)
  have <- intersect(names(df), mcq_keep)
  if (!length(have)) return(tibble(seqn = as.integer(df$seqn)))
  df %>%
    select(all_of(have)) %>%
    mutate(seqn = as.integer(seqn))
})

# Recode to 1/0 (1/Yes=1; No/Refused/DK/Missing=0) and sum
if (nrow(mcq_all) > 0) {
  mcq_item_names <- setdiff(names(mcq_all), "seqn")
  mcq_clean <- mcq_all %>%
    mutate(across(all_of(mcq_item_names), mcq_yes1_else0)) %>%
    mutate(comorbidity_n = rowSums(across(all_of(mcq_item_names)), na.rm = TRUE)) %>%
    select(seqn, comorbidity_n)
} else {
  mcq_clean <- tibble(seqn = integer(), comorbidity_n = integer())
}

# HIQ: insurance (Yes/No/Unknown) 
...
model_vars <- c("age_years","sex","race_eth","ed_cat","indfmpir_cat",
...
# Quick audit
print(missingness_summary(analytic %>% select(age_years, sex, race_eth, ed_cat, indfmpir, indfmpir_cat,
...
tbl1_by_outcome <- gtsummary::tbl_summary(
  ...
  include = c(
    poly_cat,              
    rxdcount, age_years, sex, race_eth, ed_cat, indfmpir_cat, insured,
    comorbidity_n
  ),
  label = list(
    age_years     ~ "Age",
    ...
    comorbidity_n ~ "Number of comorbidity",
...
tbl1_by_exposure <- gtsummary::tbl_summary(
  ...
  by = poly_cat,
  include = c(
    age_years, sex, race_eth, ed_cat, indfmpir_cat, insured,
...
m_basic_svy <- svycoxph(
  Surv(time_y, event) ~ poly_cat + age_years + sex + race_eth,
  design = dsgn
)
...
m_spline_svy <- svycoxph(
  Surv(time_y, event) ~ splines::ns(rxdcount, knots = c(5, 10)) +
    age_years + sex + race_eth + ed_cat + indfmpir_cat + insured + comorbidity_n,
  design = dsgn
)
...
ref_row <- tibble::tibble(
  age_years     = median(analytic_final$age_years, na.rm = TRUE),
